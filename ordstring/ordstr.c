...
//////////////////////////////////////////////////////
void ordenar(lista_string* l){
//	l->string;
	tamanho = l->quantidade;
        int i, cont=0;
	char* aux;
        ...
        while (valida == FALSE)
        {
                ...
                {
                        if (strcmp(l->string[i],l->string[i+1])>0)
                        {
				aux = (char*) malloc(sizeof(char)*strlen(l->string[i])+1);
                                strcpy(aux, l->string[i]);
                              	strcpy(l->string[i], l->string[i+1]);
				strcpy(l->string[i+1], aux);
                                free(aux);
                                cont++;
                        }
